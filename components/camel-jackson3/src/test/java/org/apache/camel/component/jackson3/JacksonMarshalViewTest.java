    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                ...
                from("direct:inPojoAgeView").marshal().json(JsonLibrary.Jackson3, TestPojoView.class, Views.Age.class);
                // END SNIPPET: format
                from("direct:backPojoAgeView").unmarshal().json(JsonLibrary.Jackson3, TestPojoView.class)
                        .to("mock:reversePojoAgeView");
                from("direct:inPojoWeightView").marshal().json(JsonLibrary.Jackson3, TestPojoView.class, Views.Weight.class);
                from("direct:backPojoWeightView").unmarshal().json(JsonLibrary.Jackson3, TestPojoView.class)
